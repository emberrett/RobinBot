...
class TestRetrievalMethods(unittest.TestCase):
    ...
    def test_get_average_cost(self):
        ...
        self.assertIsInstance(self.robin_bot.get_average_cost(portfolio_ticker_symbol), float)

    def test_get_historical_prices(self):
        self.assertIsInstance(self.robin_bot.get_historical_prices('AAPL'), list)

    def test_get_current_price(self):
        self.assertIsInstance(self.robin_bot.get_current_price('AAPL'), float)

    def test_get_price_change(self):
        self.assertIsInstance(self.robin_bot.get_price_change('AAPL'), float)
    
    def test_get_price_changes(self):
        self.assertIsInstance(self.robin_bot.get_price_changes(['AAPL', 'AMZN', 'GOOGL']), dict)
    ...
    def test_sort_top_movers(self):
        ...
        first_negative_ticker = next(iter(self.robin_bot.sort_top_movers(top_n_market_movers, False).values()))
        self.assertLess(first_negative_ticker, 0)

        first_positive_ticker = next(iter(self.robin_bot.sort_top_movers(top_n_market_movers).values()))
        self.assertGreater(first_positive_ticker, 0)

    def test_get_symbol_equity(self):
        portfolio_ticker_symbol = self.robin_bot.get_portfolio_symbols()[0]
        self.assertIsInstance(self.robin_bot.get_symbol_equity(portfolio_ticker_symbol), float)
    ...
    def test_get_shares(self):
        portfolio_ticker_symbol = self.robin_bot.get_portfolio_symbols()[0]
        self.assertIsInstance(self.robin_bot.get_shares(portfolio_ticker_symbol), float)

    def test_get_total_in_robinhood(self):
        portfolio_ticker_symbol = self.robin_bot.get_portfolio_symbols()[0]
        self.assertIsInstance(self.robin_bot.get_total_in_robinhood(portfolio_ticker_symbol), float)
